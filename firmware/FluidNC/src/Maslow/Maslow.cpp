...
#include "../Config.h"  // For X_AXIS, Y_AXIS, Z_AXIS constants
#include <cmath>
...
void Maslow_::update() {
    ...
    if (!Maslow.using_default_config) {
        ...
        if (sys.state() == State::Jog || sys.state() == State::Cycle) {
            ...
            // Apply work area constraints by computing current X,Y and constraining belt lengths
            using namespace Kinematics;
            MaslowKinematics* kinematics = getMaslowKinematics();
            if (kinematics) {
                // Convert belt lengths to X,Y position
                float motors[MAX_N_AXIS] = {tlBeltLength, trBeltLength, blBeltLength, brBeltLength, zPosition, 0};
                float cartesian[MAX_N_AXIS] = {0};
                kinematics->motors_to_cartesian(cartesian, motors, 5);

                float currentX = cartesian[X_AXIS];
                float currentY = cartesian[Y_AXIS];

                // Calculate work area bounds
                float halfWidth = workAreaX / 2.0f;
                float halfHeight = workAreaY / 2.0f;
                float minX = -halfWidth + workAreaCenterOffsetX;
                float maxX = halfWidth + workAreaCenterOffsetX;
                float minY = -halfHeight + workAreaCenterOffsetY;
                float maxY = halfHeight + workAreaCenterOffsetY;

                // Constrain X and Y to work area
                bool constrained = false;
                if (currentX < minX) {
                    cartesian[X_AXIS] = minX;
                    constrained = true;
                } else if (currentX > maxX) {
                    cartesian[X_AXIS] = maxX;
                    constrained = true;
                }

                if (currentY < minY) {
                    cartesian[Y_AXIS] = minY;
                    constrained = true;
                } else if (currentY > maxY) {
                    cartesian[Y_AXIS] = maxY;
                    constrained = true;
                }

                // If position was constrained, recompute belt lengths from constrained position
                if (constrained) {
                    float constrainedMotors[MAX_N_AXIS] = {0};
                    kinematics->transform_cartesian_to_motors(constrainedMotors, cartesian);
                    tlBeltLength = constrainedMotors[0];
                    trBeltLength = constrainedMotors[1];
                    blBeltLength = constrainedMotors[2];
                    brBeltLength = constrainedMotors[3];
                }
            }
